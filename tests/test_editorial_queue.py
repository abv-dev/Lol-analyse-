...
def seed(db, patch, version, matches, days_ago, roles=True, grubs=False, start=0):
    ...
    cur = db.conn.cursor()
    # Colonnes nommées : le schéma gagne des colonnes (early_surrender,
    # métriques du Lot 13) sans que ces tests aient à bouger.
    cur.executemany("INSERT OR IGNORE INTO matches (match_id, region, platform,"
                    " game_version, patch, game_duration, game_creation,"
                    " tier_bucket_source, inserted_at) VALUES (?,?,?,?,?,?,?,?,?)", m)
    cur.executemany("INSERT INTO participants (match_id, puuid, champion_id,"
                    " champion_name, team_id, team_position, win, kills, deaths,"
                    " assists, item0, item1, item2, item3, item4, item5, item6,"
                    " perk_primary_style, perk_sub_style, perk_keystone,"
                    " gold_earned, total_cs, patch)"
                    " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", p)
    cur.executemany("INSERT INTO team_objectives (match_id, team_id, first_blood,"
                    " first_tower, first_dragon, first_baron, dragon_kills,"
                    " baron_kills, tower_kills, herald_kills, horde_kills)"
                    " VALUES (?,?,?,?,?,?,?,?,?,?,?)", o)
    db.conn.commit()
...
# --- 1) gabarits : le catalogue s'instancie pour un patch ------------------
#
# Les comptes ne sont plus figés dans le test : le catalogue grossit (il est
# passé de 11 gabarits / 15 articles à la cadence quotidienne). Ce qui est
# vérifié, c'est la mécanique — une variante par article, identifiants et
# slugs uniques, paramètres de génération transportés.

created = instantiate(TEMPLATES, "16.15")
expected = sum(len(t.get("variantes") or [{}]) for t in TEMPLATES)
assert len(created) == expected, (len(created), expected)
assert len(created) >= 40, f"catalogue trop maigre pour une publication quotidienne : {len(created)}"
assert len({a["id"] for a in created}) == len(created), "identifiants en double"
assert len({a["slug"] for a in created}) == len(created), "slugs en double"
assert len({a["titre"] for a in created}) == len(created), "titres en double"
...
assert roles[0]["slug"] == "meta-top/16-15", roles[0]["slug"]
assert all(a["patch_cible"] == "16.15" for a in created)
assert all(a["generateur"] for a in created), "un gabarit sans générateur"
assert roles[0]["params"].get("role") == "TOP", roles[0]["params"]
print(f"OK  {len(TEMPLATES)} gabarits -> {len(created)} articles pour 16.15, "
      f"identifiants, slugs et titres uniques")

# --- 2) sync idempotent -----------------------------------------------------

queue = {"articles": []}
assert len(sync_templates(queue, TEMPLATES, "16.15")) == expected
assert sync_templates(queue, TEMPLATES, "16.15") == []
assert len(queue["articles"]) == expected
...
assert len(sync_templates(queue, TEMPLATES, "16.16")) == expected
