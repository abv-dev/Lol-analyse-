...
# ELOLAB_REPO permet aux tests (et à une copie de travail) de pointer une
# autre racine que celle du paquet.
REPO = os.environ.get(
    "ELOLAB_REPO", os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
DATA_CHECKS = {
    ...
    # Colonnes des lots 13 et 14 : NULL sur tout l'historique antérieur à
    # leur migration, donc comptées non NULL. Le COUNT est plafonné par un
    # LIMIT à l'intérieur de la sous-requête : on veut savoir si le volume
    # est atteint, pas compter 6 millions de lignes à chaque passage.
    "vision_score": (
        "vision scores renseignés",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND vision_score IS NOT NULL LIMIT 100000)", 100_000),
    "damage_to_champions": (
        "dégâts renseignés",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND damage_to_champions IS NOT NULL LIMIT 100000)", 100_000),
    "gold_earned": (
        "or gagné renseigné",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND gold_earned IS NOT NULL AND gold_earned > 0 LIMIT 100000)", 100_000),
    "cs_at10": (
        "CS à 10 minutes renseignés",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND lane_cs_at10 IS NOT NULL LIMIT 100000)", 100_000),
    "time_spent_dead": (
        "temps passé mort renseigné",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND time_spent_dead IS NOT NULL LIMIT 100000)", 100_000),
    "control_wards_bought": (
        "pinks renseignés",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND control_wards_bought IS NOT NULL LIMIT 100000)", 100_000),
    "turret_plates_taken": (
        "plates renseignées",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND turret_plates_taken IS NOT NULL LIMIT 100000)", 100_000),
    "participant_id": (
        "identifiants de participant (jointure timeline)",
        "SELECT COUNT(*) FROM (SELECT 1 FROM participants WHERE {cond}"
        " AND participant_id IS NOT NULL LIMIT 100000)", 100_000),
    "timelines_collectees": (
        "matchs dont la timeline est collectée",
        "SELECT COUNT(*) FROM (SELECT 1 FROM matches m"
        " JOIN timeline_state t ON t.match_id = m.match_id AND t.status = 'ok'"
        " WHERE {cond} LIMIT 20000)", 20_000),
}

# Les tables jointes préfixent leur colonne patch, les autres non.
PATCH_COND = {
    "bans": "m.patch = ?", "timelines_collectees": "m.patch = ?", "timeline_events": "m.patch = ?",
    ...
    for tpl in templates:
        ...
        for variant in variants:
            ...
            created.append({
                ...
                "tentatives": 0,
                "gabarit": tpl["id"],
                # Ce qui relie l'article au calcul : le type de construction
                # (lolcollector.studygen.BUILDERS) et ses paramètres, la
                # variante complétant ceux du gabarit.
                "generateur": tpl.get("generateur"),
                "params": {**(tpl.get("params") or {}),
                           **(variant.get("params") or {})},
                "tags": list(variant.get("tags") or tpl.get("tags") or []),
            })
    return created
